...
LV_EVENT_CB_DECLARE(icon_yg_img_1_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_4_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_6_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_5_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_9_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_8_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_2_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_3_event_cb);
LV_EVENT_CB_DECLARE(icon_yg_img_7_event_cb);
...
static void(*lv_yg_event_cb[])(lv_obj_t * obj, lv_event_t e) = \
{
	icon_yg_img_1_event_cb, icon_yg_img_4_event_cb, \ 
	icon_yg_img_6_event_cb, icon_yg_img_5_event_cb,\ 
	icon_yg_img_9_event_cb, icon_yg_img_8_event_cb, \ 
	icon_yg_img_2_event_cb, icon_yg_img_3_event_cb,\ 
	icon_yg_img_7_event_cb
};
 ...
 *	 GLOBAL FUNCTIONS
 **********************/
void lv_wingcool_boot_animation(uint32_t delay)
{
	...
	lv_label_set_text(title, "wingcool.cn");
	
	//	lv_label_set_align(title, LV_LABEL_ALIGN_CENTER);
	lv_obj_set_style_local_text_font(title, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, &lv_font_montserrat_14);
	lv_obj_set_style_local_bg_opa(title, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_COVER); // 设置色彩透明度
	lv_obj_set_style_local_text_color(title, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x000000)); //设置颜色
	
	//	lv_obj_set_style_local_text_line_space(title, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, 8);
	lv_obj_align(title, logo, LV_ALIGN_OUT_BOTTOM_RIGHT, -20, -10);
	lv_obj_fade_out(title, 0, delay);
	lv_obj_fade_out(logo, 0, delay);
}


static void lv_wingcool_start_animation(void)
{
	static uint8_t first_pw_on = 0;
	...
	//	  lv_obj_set_style_local_text_line_space(title, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, 5);
	lv_obj_align(title, logo, LV_ALIGN_OUT_BOTTOM_RIGHT, -20, -10);
	
	if(first_pw_on == 0)
	{
		first_pw_on = 1;
		lv_obj_fade_out(title, 2000, 0);
		lv_obj_fade_out(logo, 2000, 0);
	}
}
...
void lv_wingcool_demo_printer(lv_indev_t * indev)
{
	...
	//	  lv_theme_t * th = lv_wingcool_demo_theme_init(LV_COLOR_BLACK, LV_COLOR_BLACK,
	...
	//	lv_wingcool_start_animation();					/* start logo */
	lv_wingcool_boot_animation(2000);
	home_open(2010);
}
...
void home_open(uint32_t delay)
{
	...
	lv_img_set_src(img, &sky_320x240);
	...
	//	lv_page_set_scroll_propagation(tabview_desktop, true);
	//	 lv_page_set_scrollbar_mode(tabview_desktop, LV_SCROLLBAR_MODE_OFF);
	...
	lv_page_set_scrlbar_mode(tab_left, LV_SCROLLBAR_MODE_OFF);
	lv_page_set_scrlbar_mode(tab_main, LV_SCROLLBAR_MODE_OFF);
	lv_page_set_scrlbar_mode(tab_right, LV_SCROLLBAR_MODE_OFF);
	...
	//	  lv_obj_align_origo(icon, NULL, LV_ALIGN_IN_TOP_LEFT, 0, -90);
	//	  lv_yg_demo_anim_top_to_bottom(icon, delay);
	...
	lv_obj_align(right_round1, NULL, LV_ALIGN_CENTER, 0, 110);
	...
	lv_obj_align(right_round2, NULL, LV_ALIGN_CENTER, 20, 110);
	...
	lv_obj_align(right_round3, NULL, LV_ALIGN_CENTER, -20, 110);
	
	for(int i = 0; i < MENU_ITEM_NUM; i++)
	{
		/* 图标 */
		static lv_obj_t * icon;
		static lv_obj_t * label_app_name;
		...
		lv_obj_set_drag(icon, false);				// 启用拖动
		...
		if(i == 0)
		{
			lv_obj_align(icon, NULL, LV_ALIGN_CENTER, -80, -60); // 设置摆放位置
		}
		else if(i == 1)
		{
			lv_obj_align(icon, NULL, LV_ALIGN_CENTER, 80, -60); // 设置摆放位置
		...
		{
			lv_obj_align(icon, NULL, LV_ALIGN_CENTER, -80, 0); // 设置摆放位置
		}
		else if(i == 3)
		{
			lv_obj_align(icon, NULL, LV_ALIGN_CENTER, 80, 0); // 设置摆放位置
		...
		lv_obj_set_event_cb(icon, lv_yg_event_cb[i]); /* 分配回调处理函数 */
		/* 创建 app name 标签 */
...
void lv_wingcool_demo_anim_top_to_bottom(lv_obj_t * obj, uint32_t delay)
{
	if(obj != bg_top && obj != lv_scr_act())
	{
		...
		lv_anim_set_time(&a, LV_WINGCOOL_DEMO_ANIM_TIME_HOME);
		lv_anim_set_delay(&a, delay);
		lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t) lv_obj_set_y);
		lv_anim_set_values(&a, lv_obj_get_y(obj) - LV_WINGCOOL_DEMO_ANIM_Y, lv_obj_get_y(obj));
		lv_anim_start(&a);
		lv_obj_fade_in(obj, LV_WINGCOOL_DEMO_ANIM_TIME - 50, delay);
	}
}
...
void lv_wingcool_demo_anim_bg(uint32_t delay, lv_color_t color, int32_t y_new)
{
	...
	if(y_new != LV_WINGCOOL_DEMO_BG_NORMAL && y_new == y_act && act_color.full == color.full)
	{
		return;
	}
	...
	if((y_new == LV_WINGCOOL_DEMO_BG_NORMAL && y_new == y_act) || (y_new == LV_WINGCOOL_DEMO_BG_NORMAL &&
	        y_act == LV_WINGCOOL_DEMO_BG_FULL))
	{
		lv_anim_path_t path;
		...
		lv_anim_set_time(&a, LV_WINGCOOL_DEMO_ANIM_TIME_BG + 200);
		lv_anim_set_delay(&a, delay);
		lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t) lv_obj_set_y);
	...
	else
	{
		lv_anim_init(&a);
		lv_anim_set_var(&a, bg_top);
		lv_anim_set_time(&a, LV_WINGCOOL_DEMO_ANIM_TIME_BG);
		lv_anim_set_delay(&a, delay);
		lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t) lv_obj_set_y);
		...
		if(child != bg_top && child != lv_scr_act())
		{
			...
			if(lv_obj_get_y(child) < 80)
			{
				lv_anim_set_values(&a, lv_obj_get_y(child),
				    lv_obj_get_y(child) - LV_WINGCOOL_DEMO_ANIM_Y);
			}
			else
			{
				lv_anim_set_values(&a, lv_obj_get_y(child),
				    lv_obj_get_y(child) + LV_WINGCOOL_DEMO_ANIM_Y);
				delay += LV_WINGCOOL_DEMO_ANIM_DELAY;
			}
...
static void demo(void)
{
	...
	lv_wingcool_demo_anim_top_to_bottom(back, delay);
}
...
LV_EVENT_CB_DECLARE(icon_yg_img_7_event_cb)
{
	if(e == LV_EVENT_CLICKED)
	{
		in_desktop_page = false;
		lv_demo_printer_anim_out_all(lv_scr_act(), 0);
		lv_wingcool_game_2048(my_indev);
	}
}
...
static void scan1_open(const char * btn_txt)
{
	...
	else if(strcmp(btn_txt, "SAVE") == 0)
	{
		...
		lv_obj_set_style_local_bg_color(next_btn, LV_OBJ_PART_MAIN, LV_STATE_PRESSED,
		    lv_color_darken(LV_DEMO_PRINTER_GREEN, LV_OPA_20));
	}
...
LV_EVENT_CB_DECLARE(usb_icon_event_cb)
{
	if(e == LV_EVENT_CLICKED)
	{
		...
		{
			"Contract 12.pdf", "Scanned_05_21.pdf", "Photo_132210.jpg", "Photo_232141.jpg",
		...
		lv_obj_set_style_local_text_font(print_cnt_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT,
		...
		lv_obj_set_style_local_value_font(print_btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT,
		...
		lv_obj_set_style_local_bg_color(print_btn, LV_OBJ_PART_MAIN, LV_STATE_PRESSED,
		    lv_color_darken(LV_DEMO_PRINTER_GREEN, LV_OPA_20));
...
static lv_anim_value_t anim_path_triangle(const lv_anim_path_t * path, const lv_anim_t * a)
{
	...
	else
	{
		if(a->act_time < a->time / 2)
		{
			...
			new_value = (int32_t)
			    step * (LV_DEMO_PRINTER_BG_SMALL - a->start);
		...
		else
		{
			...
			new_value = (int32_t)
			    step * (a->end - LV_DEMO_PRINTER_BG_SMALL);
			new_value = new_value >> 10;
...
static void lv_demo_printer_anim_bg(uint32_t delay, lv_color_t color, int32_t y_new)
{
	...
	if((y_new == LV_DEMO_PRINTER_BG_NORMAL && y_new == y_act) || (y_new == LV_DEMO_PRINTER_BG_NORMAL &&
	        y_act == LV_DEMO_PRINTER_BG_FULL))
	{
		lv_anim_path_t path;
