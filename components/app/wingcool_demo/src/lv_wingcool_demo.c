...
void lv_wingcool_demo_anim_top_to_bottom(lv_obj_t * obj, uint32_t delay)
{
    if (obj != bg_top && obj != lv_scr_act()) {
        ...
        lv_anim_set_values(&a, lv_obj_get_y(obj) - LV_WINGCOOL_DEMO_ANIM_Y,
               lv_obj_get_y(obj));
        lv_anim_start(&a);
...
void lv_wingcool_demo_anim_bg(uint32_t delay, lv_color_t color, int32_t y_new)
{
    ...
    if(y_new != LV_WINGCOOL_DEMO_BG_NORMAL && y_new == y_act && act_color.full == color.full) return;


    lv_anim_t a;
    if((y_new == LV_WINGCOOL_DEMO_BG_NORMAL && y_new == y_act) ||
            (y_new == LV_WINGCOOL_DEMO_BG_NORMAL && y_act == LV_WINGCOOL_DEMO_BG_FULL)) {
        lv_anim_path_t path;
